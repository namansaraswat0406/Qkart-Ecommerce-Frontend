    ...
    <Box display="flex" flexDirection="column">
      ...
      <Stack direction="row" my="1rem">
        <Button
          variant="contained"
          onClick={() => {
            addAddress(token, currentAddress);
          }}
        >
          Add
        </Button>
...
const Checkout = () => {
  ...
  const getAddresses = async (token) => {
    ...
    try {
      ...
      // console.log("Milan 1111 : " + JSON.stringify(response));
      return response.data;
    } catch {
      enqueueSnackbar(
  ...
  //     });
  //   } else {
  //     console.log(await performCheckout(localStorage.getItem("token"), items, addresses))
  //     if(await performCheckout(localStorage.getItem("token"), items, addresses)) console.log("sab chl rha h !!")
  //   }
  // };
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try{
      let addressID = addresses.all.filter((val) => val.address === addresses.selected)
      // console.log(addressID[0]._id)
      let response = await axios.post(
  ...
  useEffect(() => {
    const onLoadHandler = async () => {
      ...
      if (token) {
        ...
        // console.log(token);
        // console.log(
        //   "Milan there are the fetched addresses : " + JSON.stringify(addresses)
        // );
      } else {
          ...
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              ...
              {addresses.all.map((val) => {
                // console.log("condition true ? : " + JSON.stringify(addresses));
                // console.log(token);
                if (addresses.selected === val.address) {
                  return (
                    <div className="address-item selected" key={val._id}>
                      <div>{val.address}</div>
                      <div>
                        <Button variant="outlined" startIcon={<DeleteIcon />} onClick={() => deleteAddress(token, val._id)}>
                          Delete
                        </Button>
                      </div>
                    </div>
